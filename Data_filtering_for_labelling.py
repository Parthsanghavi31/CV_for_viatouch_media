...
def get_frames_from_video(video_path, frames):
...
def process_frames_v2(original_frames, subtracted_images, canny_images, diff_canny_images):
...
def processing_norms(subtracted_images, diff_canny_images, canny_norms, sub_img_norms):
...
def scaled_norms(sub_norms, canny_norms): #input is a list
    ...
    frames_of_interest = np.where(digital_signal>0)[0] #Adding 10 because we start from 10th frame.
    # print(frames_of_interest)
    
    return scaled_sub_norm.tolist(), scaled_canny_norm.tolist(), digital_signal, frames_of_interest

def crop_frames(frames, cropped_frames, cropping):

    for i in range(1,len(frames)-1):
        cropped_frame = frames[i][60:120, :] 
        cropped_frames.append(cropped_frame)
    if cropping == True:
        return cropped_frames
    return frames

def save_filtered_frames(cropped_frames, frames_of_interest, save_path):
    cropped_frames = np.array(cropped_frames)
    alternate_frames_of_interest = frames_of_interest[::2]
    filtered_images = cropped_frames[alternate_frames_of_interest]
    for i,frame in enumerate(filtered_images):
        file_name = f"frame_{i}.jpg"
        frame_path = os.path.join(save_path, file_name)
        cv2.imwrite(frame_path, frame)
    return filtered_images

def main():
    video_dir = "office_videos_data"
    save_dir = "/home/parth/Viatouch_Media_Projects/Motion_detection/modified_frames"
    ...
    for vid_idx, video_file in enumerate(video_files):
        frames_list = [] 
        subtracted_images_lit = []
        canny_images_list = [] 
        diff_canny_images_list = [] 
        sub_norms_list = []
        canny_norms_list = [] 
        cropped_frames_list = []
        ...
        frames = get_frames_from_video(video_path, frames_list)
        frames = crop_frames(frames, cropped_frames_list, cropping=True, )
        
        start_idx = 10
        end_idx = len(frames) - 10
        if end_idx <= start_idx:
            print("Not enough frames after cropping; skipping this video.")
            continue

        valid_frames = frames[start_idx:end_idx]
        original_frames, subtracted_images, canny_images, diff_canny_images = process_frames_v2(valid_frames, subtracted_images_lit, canny_images_list, diff_canny_images_list)
        sub_norms, canny_norms = processing_norms(subtracted_images, diff_canny_images, canny_norms_list, sub_norms_list)
        scaled_sub_norm, scaled_canny_norm, digital_signal, frames_of_interest = scaled_norms(sub_norms, canny_norms)

        # Use unique folder per video (or prefix)
        video_name = os.path.splitext(video_file)[0]
        video_save_dir = os.path.join(save_dir, f"{video_name}_frames")
        os.makedirs(video_save_dir, exist_ok=True)

        filtered_images = save_filtered_frames(valid_frames, frames_of_interest, video_save_dir)
        ...
        print(f"Saved {filtered_images.shape[0]} frames to {video_save_dir}")
