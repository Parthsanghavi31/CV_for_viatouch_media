...
def crop_frames(frames, cropping, cropped_frames = []):
    ...
    for i in range(1,len(frames)-1):
        ...
        cropped_frame = frames[i][85:134, :] # for Test_vid1 videos
...
def process_frames(frames, norms = [], subtracted_images= []):
    ...
    for idx in range(10, len(frames)-10):
        ...
        norm = np.linalg.norm(subtracted_image)
        norms.append(norm)
        smoothed_norms  = medfilt(np.array(norms), kernel_size=3)
        
            
    return subtracted_images, list(smoothed_norms)
